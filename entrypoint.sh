...
if [ "$VNC" = "1" ]; then
  ...
  # Write xstartup that keeps the session alive
...
SH
  ...
  # vnc config (let xstartup control session)
  ...
  # start vncserver (ensure HOME is APP_HOME)
  if [ "$is_root" -eq 1 ]; then
  ...
  # wait for :5901 to come up (use nc instead of ss)
  for _ in {1..20}; do
